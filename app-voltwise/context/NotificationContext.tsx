...
/**
 * Notification preferences + in-app alert engine.
 *
 * Runs purely in JS: tracks new alerts, updates badges and feeds,
 * and triggers haptic feedback when alerts arrive while the app is active.
 */

export type PermissionState = "granted" | "denied" | "undetermined" | "unsupported";

/** Never process more than this many missed alerts at once after a gap. */
...
interface NotificationContextValue {
  ...
  /** Master + severity + quiet hours — for in-app alert triggers, sound and haptics. */
...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
  ...
  const [permission] = useState<PermissionState>("granted");
  ...
  // Highest alert id already processed.
  ...
  /**
   * Diff GET /alerts against the high-water mark.
   *
  ...
  const sync = useCallback(
    async function run(baselineOnly = false): Promise<void> {
      ...
      try {
        ...
        // processed twice by the three redundant refresh subscribers.
        hwmRef.current = highWaterMark(list, hwmRef.current);
        saveNotifiedHighWater(hwmRef.current).catch(() => {});

        if (!baselineOnly) {
          // After a long gap, process only the most recent few.
